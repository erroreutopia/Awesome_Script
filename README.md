...
* agamepack：打包PRGMaker,Wine为Appimage（注：RPG存档位置为：$HOME/Game/HTMLGame/NWJS/$(GAMENAME)/），wine如果需要在游戏目录而非.wine目录会无法保存！以及，Nwjs和proton-ge前者需要在$HOME/App/nwjs-sdk/,后者需要在`PATH`
使用方法：agamepack -r 目录，没有默认为game目录。-i icon文件。 -n 游戏名称。 --wine-exec 设置wine的可执行文件名，但是脚本会自动识别，可无视。--wine-cmd 设置wine/proton等兼容层路径，默认为proton-ge。
